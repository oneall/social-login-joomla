	...
	if (is_array ($manifest->modules))
	{
		...
		foreach ($manifest->modules AS $mlvl1)
		{
			foreach ($mlvl1->children () as $module)
			{
				...
				if ($this->parent->parseFiles ($module->files[0], -1) === false)
				{
		...
		foreach ($manifest->plugins AS $plvl1)
		{
			foreach ($plvl1->children () as $plugin)
			{
				...
				if (empty ($plugin_id))
				{
					...
					$data ['client_id'] = 0;
					$data ['published'] = 1;
					$data ['showtitle'] = (!empty ($plg_data ['showtitle']) ? 1 : 0);
					$data ['ordering'] = $plg_data ['order'];
					$data ['iscore'] = 0;
					$data ['access'] = 0;
					$data ['params'] = '{}';
