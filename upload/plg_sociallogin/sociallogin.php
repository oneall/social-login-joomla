	...
	private function doAuth ($token)
	{
		...
		if (is_object ($social_data))
		{
			...
			if (!is_numeric ($user_id))
			{
				...
				$new_user_type = $usersParams->get ('new_usertype');
				if ( ! $new_user_type)
				{
					$new_user_type = 'Registered';
				}
				...
				$data ['usertype'] = '';
				$data ['gid'] = $authorize->get_group_id( '', $new_user_type, 'ARO' );
			...
			if (isset ($user_id) AND is_numeric ($user_id) AND !empty ($user_id))
			{
				...
				if (is_object ($user))
				{
					...
					if ($result AND ! empty ($result->username))
					{
						...
						$result = $app->triggerEvent ('onLoginUser', array (
							(array) $response,
							$options
						));
